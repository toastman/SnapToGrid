$.ui.plugin.add("draggable", "collide", {
    start: function(event, ui) {
        ...
        widget.snapStep = widget.options.snapStep;
    ...
    drag: function(event, ui) {
        ...
        $(widget.peers).each(function (i, peer) {
            ...
            // Test for overlap
            if (((ix1 <= px1 && ix2 >= px1) || (ix1 <= px2 && ix2 >= px2) || (ix1 >= px1 && ix2 <= px2)) &&
                ((iy1 <= py1 && iy2 >= py1) || (iy1 <= py2 && iy2 >= py2) || (iy1 >= py1 && iy2 <= py2))) {
                if (widget.mode == 'flag') {
                    ...
                    if (Math.abs(overlapx) > Math.abs(overlapy)) {
                        if (iy1 <= py1) ui.position.top = snap(py1 - $t.height());
                        if (iy1 > py1) ui.position.top = snap(py2);
                    } else {
                        if (ix1 <= px1) ui.position.left = snap(px1 - $t.width());
                        if (ix1 > px1) ui.position.left = snap(px2);
                    }
                }
            } else {
                if (widget.mode == 'flag') {
      ...
      function snap(value){
        return widget.snapStep * Math.round(value / widget.snapStep);
      }
    }
});
